...
public class PatientService implements IPatientService {
    ...
    public PatientDTO insert(PatientDTO dto) {
        return modelMapper.map(patientRepository.save(modelMapper.map(dto, Patients.class)), PatientDTO.class);
    }
    ...
    public PatientDTO update(PatientDTO dto) throws BadActionException {
        Optional<Patients> patient = patientRepository.findById(dto.getId());
        if (patient.isEmpty()) {
            throw new BadActionException("Patient not found with id: " + dto.getId());
        }
        if (dto.getPatientCode() != null) {
            patient.get().setPatientCode(dto.getPatientCode());
        }
    ...
    @Override
    public PaginationResultDTO getAll(Specification<Patients> spec, Pageable pageable) {
        // TODO Auto-generated method stub
